...
setups: point `CC_SUDO_ASKPASS` at your own helper (see Configuration).

## Verify (agent, shell)

Feed the hook a sample Bash tool call and confirm it rewrites `sudo` (no sudo
...
askpass exists so a failing probe isn't misread as "no sudo here".
